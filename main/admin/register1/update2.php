...
$statement = $pdo->prepare("UPDATE requests_material SET status=? WHERE varietyName=?");
$statement->execute(array($_REQUEST['task'],$_REQUEST['varietyName']));

// Get the request details
$stat = $pdo->prepare("SELECT * FROM requests_material WHERE varietyName=?");
...
header('Location:requestedp.php');
