        ...
        <div class="box-body table-responsive">
          <table id="example1" class="table table-bordered table-hover table-striped">
            ...
            <tbody>
            	<?php
            	$i=0;
            	$statement = $pdo->prepare("SELECT * FROM requests_material ORDER by id DESC");
            	$statement->execute();
            	$result = $statement->fetchAll(PDO::FETCH_ASSOC);							
            	foreach ($result as $row) {
            		$i++;
                    ...
                    <?php echo $row['inventoryStatus']; ?>                      
                        ...
                        <td>
                    <?php echo $row['prodstats']; ?>                      
                        </td>
                        <td>
                        <?php
                                if($row['inventoryStatus']=='Pending'){
                                    if($row['status']=='Released'){
                                        if($row['prodstats']=='Completed'){
                                    ?>                                 
                                   <a href="updatep3.php?p_name=<?php echo $row['p_name']; ?>&task=Quantity Updated" class="btn btn-primary btn-lg" >Update Quantity</a>
                                    <?php
                                }}}
                            ?>                     
                        </td>
