	...
	if($_SESSION['user']['role'] == 'Super Admin') {
	    ...
	    if(empty($_POST['email'])) {
	    	...
	    	if (filter_var($_POST['email'], FILTER_VALIDATE_EMAIL) === false) {
		    	...
		    	$statement = $pdo->prepare("SELECT * FROM tbl_staff WHERE id=?");
		    	...
		    	$statement = $pdo->prepare("SELECT * FROM tbl_staff WHERE email=? and email!=?");
	    ...
	    if($valid == 1) {
			...
			$statement = $pdo->prepare("UPDATE tbl_staff SET full_name=?, email=?, phone=? WHERE id=?");
		...
		// updating the database
		$statement = $pdo->prepare("UPDATE tbl_staff SET phone=? WHERE id=?");
...
if(isset($_POST['form2'])) {
    ...
    if($valid == 1) {
        ...
        // updating the database
		$statement = $pdo->prepare("UPDATE tbl_staff SET photo=? WHERE id=?");
...
if(isset($_POST['form3'])) {
    ...
    if($valid == 1) {
    	...
    	// updating the database
		$statement = $pdo->prepare("UPDATE tbl_staff SET password=? WHERE id=?");
