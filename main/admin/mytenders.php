        ...
        <div class="box-body table-responsive">
          <table id="example1" class="table table-bordered table-hover table-striped">
            ...
            <tbody>
            	<?php
            	$i=0;
            	$statement = $pdo->prepare("SELECT * FROM tbl_tender_application");
            	$statement->execute();
            	$result = $statement->fetchAll(PDO::FETCH_ASSOC);							
            	foreach ($result as $row) {
            		$i++;
					...
					<tr class="<?php if($row['payment_status']=='Pending'){echo 'bg-r';}else{echo 'bg-g';} ?>">
	                    ...
	                    <td>
                        <?php
                           $statement1 = $pdo->prepare("SELECT * FROM tbl_tender WHERE id=?");
                           $statement1->execute(array($row['cust_id']));
                           $result1 = $statement1->fetchAll(PDO::FETCH_ASSOC);
                           foreach ($result1 as $row1) {
                                echo '<b>Product Name:</b> '.$row1['product_name'];
                                echo '<br><b>Requested Quantity:</b> '.$row1['quantity'];
                                echo ', <br><b>Description:</b> '.$row1['description'].'';
                                echo '<br><br>';
                           }
                           ?>
                        </td>
                        <td>  <b>Price Per Piece :</b><?php echo $row['price']; ?><br>
                        <b>Total Amount :</b><?php echo $row['total_amount']; ?><br>
                        </td>
                        ...
                        <td>   <?php
                                if($row['status']==''){
                                    ?>
                                    <a href="tender-change-status.php?id=<?php echo $row['id']; ?>&task=Approved" class="btn btn-success btn-xs" style="width:100%;margin-bottom:4px;">Approve</a>
                                    <?php
                                }
                            ?>
                             <?php
                                if($row['status']==''){
                                    ?>
                                    <a href="deny-tender-change-status.php?id=<?php echo $row['id']; ?>&task=Denied" class="btn btn-danger btn-xs" style="width:100%;margin-bottom:4px;">Deny</a>
                                    <?php
                                }
                            ?>
                             <?php
                                if($row['status']=='Supplied'){
                                    ?>
                                    <a href="received-change-status.php?id=<?php echo $row['id']; ?>&task=Received" class="btn btn-success btn-xs" style="width:100%;margin-bottom:4px;">Received</a>
                                    <?php
                                }
                            ?>
                        </td>
