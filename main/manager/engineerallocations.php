        ...
        <div class="box-body table-responsive">
          <table id="example1" class="table table-bordered table-hover table-striped">
            ...
            <tbody>
            	<?php
            	$i=0;
            	$statement = $pdo->prepare("SELECT * FROM tbl_bookings where service='Building Quotations' ORDER by id DESC");
            	$statement->execute();
            	$result = $statement->fetchAll(PDO::FETCH_ASSOC);							
            	foreach ($result as $row) {
            		$i++;
					...
					<tr class="<?php if($row['payment_status']=='Pending'){echo 'bg-r';}else{echo 'bg-g';} ?>">
                        ...
                        <?php
                           ...
                           foreach ($result1 as $row1) {
                                ...
                                echo '<br><b>Charges:</b> '.$row1['pricing'];
                                echo '<br><br>';
                           }
                           ?>
                        </td>
                       ...
                       <?php
                                           ...
                                           $statement1->execute(array($row['engineer']));
                                           $result1 = $statement1->fetchAll(PDO::FETCH_ASSOC);
                                           foreach ($result1 as $row1) {
                    ...
                    <td><?php echo $row['engineer_recommandation']; ?>
                    <?php
                                    if($row['landsize']!=='0'){
                                    ?>
                                    <a href="submittedquotation.php?id=<?php echo $row['id']; ?>&task=Approved" class="btn btn-primary btn-xs" style="width:100%;margin-bottom:4px;">View File Report</a>
                                    <?php
                            }
                    ...
                    <td>
                    <?php
                                if($row['engineer']=='Not Assigned'){
                                    if($row['service']=='Building Quotations'){
                                    ?>
                                    <a href="assignengineer.php?id=<?php echo $row['id']; ?>&task=Approved" class="btn btn-success btn-xs" style="width:100%;margin-bottom:4px;">Assign Engineer</a>
                                    <?php
                            }
                        }
                            ?>
