                ...
                <div class="box-body table-responsive">
                    <table id="example1" class="table table-bordered table-hover table-striped">
                        ...
                        <tbody>
                            <?php
                            $i = 0;
                            $statement = $pdo->prepare("SELECT * FROM tbl_specialorders WHERE  designer_status='Complete'AND Designer='" . $_SESSION['user']['full_name'] . "'");
                            $statement->execute();
                            $result = $statement->fetchAll(PDO::FETCH_ASSOC);
                            foreach ($result as $row) {
                                $i++;
                            ?>
                                <tr class="<?php if ($row['payment_status'] == 'pending') {
                                    ...
                                    <td>
                                        <?php
                                        if ($row['designer_return'] == 'Not Confirmed') {
                                        ?>
                                            <a href="specialreturntoolbox.php?id=<?php echo $row['id']; ?>&task=returned" class="btn btn-success btn-xs" style="width:100%;margin-bottom:4px;">Return</a>
                                        <?php
                                        }
                                        ?><br><br>
                                        <?php echo $row['designer_return']; ?>
                                    </td>
                                </tr>
                            <?php
                            }
