...
if( !isset($_REQUEST['id']) || !isset($_REQUEST['task']) ) {
	...
	$statement = $pdo->prepare("SELECT * FROM tbl_bookings WHERE id=?");
...
<?php
    ...
    $statement = $pdo->prepare("UPDATE tbl_bookings SET status=?WHERE id=?");
	$statement->execute(array($_REQUEST['task2'],$_REQUEST['id']));

	header('location: bookingpendingpayment.php');
?>